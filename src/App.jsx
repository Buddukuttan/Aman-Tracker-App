...
function App() {
  ...
  if (isLocked) return (
    <div className="h-screen flex flex-col items-center justify-center bg-background p-10 text-center space-y-10">
      <div className="w-24 h-24 bg-primary/5 rounded-[40px] flex items-center justify-center text-primary shadow-inner"><Lock className="w-12 h-12" /></div>
      <div className="space-y-3"><h2 className="text-3xl font-bold font-display tracking-tight text-foreground">Security Vault</h2><p className="text-foreground/40 text-sm font-medium">Verify credentials to access <br/> your financial portfolio.</p></div>
      <button onClick={handleUnlock} className="w-full max-w-xs py-6 bg-primary text-primary-foreground rounded-[32px] font-bold flex items-center justify-center gap-3 shadow-2xl shadow-primary/40 active:scale-95 transition-transform"><ShieldCheck className="w-6 h-6" /><span>Biometric Verification</span></button>
    </div>
  );
      ...
      <nav className="fixed bottom-0 left-0 right-0 ios-blur border-t border-foreground/5 safe-area-pb z-50 rounded-t-[40px] shadow-[0_-10px_40px_rgba(0,0,0,0.1)]">
        <div className="flex justify-around items-center h-24 max-w-md mx-auto px-10">
          ...
          <button onClick={() => setActiveTab('log')} className={`flex flex-col items-center -mt-14`}>
            <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.9 }} className={`p-6 rounded-[32px] shadow-[0_20px_50px_rgba(0,0,0,0.3)] transition-all ${activeTab === 'log' ? 'bg-primary text-primary-foreground shadow-primary/40' : 'bg-background text-foreground/40 border border-foreground/5'}`}>
