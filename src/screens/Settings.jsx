  ...
  const handleExport = async () => {
    ...
    try {
      ...
      categoriesInEntries.forEach(cat => {
        ...
        XLSX.utils.book_append_sheet(wb, wsCat, cat.substring(0, 31)); // Excel sheet name limit
      });

      XLSX.writeFile(wb, `KaChing_Expenses_${formatIST(new Date(), 'yyyy-MM-dd')}.xlsx`);
    } catch (error) {
      console.error("Export failed:", error);
      alert("Failed to export data. Please ensure you have created the Firestore Index mentioned in Tutorials.");
    } finally {
      setExporting(false);
    }
  };
  ...
  const tutorials = {
    pwa: {
      ...
      steps: [
        ...
        { text: "The app icon will now appear on your home screen" }
      ]
    },
    backtap: {
  ...
  return (
    <div className="flex flex-col h-full max-w-md mx-auto p-6 pt-12 space-y-8 pb-32">
      ...
      <div className="space-y-4">
        <h2 className="text-xs font-semibold text-foreground/50 uppercase tracking-[0.2em] ml-1">Quick Amounts (₹)</h2>
          ...
          {quickAmounts.map((amt, i) => (
            <div key={i} className="relative">
              <input
                ...
                className="w-full bg-foreground/5 rounded-2xl py-4 px-6 font-bold outline-none focus:ring-2 ring-primary/50 text-center"
              />
            </div>
          ))}
        </div>
      ...
      <div className="space-y-4">
        <h2 className="text-xs font-semibold text-foreground/50 uppercase tracking-[0.2em] ml-1">Categories</h2>
        <div className="space-y-2">
              ...
              {editingCategory === cat ? (
                <div className="flex-1 flex items-center gap-2">
                  <input
                    ...
                    className="flex-1 bg-white dark:bg-zinc-800 px-3 py-2 rounded-xl outline-none ring-1 ring-primary"
                  />
                ...
                <>
                  ...
                  <div className="flex gap-1">
                    <button
                      ...
                      className="p-2 text-foreground/30 active:text-primary transition-colors"
                    >
                    ...
                    {categories.length > 1 && (
                      <button
                        ...
                        className="p-2 text-red-500/30 active:text-red-500 transition-colors"
                      >
          ...
          <div className="flex gap-2 mt-4">
            ...
            <button
              ...
              className="bg-primary text-white p-4 rounded-2xl active:scale-95 transition-transform"
            >
      ...
      <div className="space-y-3 pt-4">
        <button
          ...
          className="w-full flex items-center justify-between bg-foreground/5 p-5 rounded-3xl font-bold active:bg-foreground/10 transition-colors"
        >
        ...
        <button
          ...
          disabled={exporting}
          className="w-full flex items-center justify-between bg-foreground/5 p-5 rounded-3xl font-bold active:bg-foreground/10 disabled:opacity-50 transition-colors"
        >
          <div className="flex items-center gap-3">
        ...
        <button
          onClick={logout}
          className="w-full flex items-center justify-between bg-red-500/10 p-5 rounded-3xl font-bold text-red-500 active:bg-red-500/20 transition-colors"
        >
          <div className="flex items-center gap-3">
      ...
      {activeTutorial && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-end sm:items-center justify-center p-4">
          <div className="bg-background w-full max-w-sm rounded-[40px] p-8 space-y-6 relative max-h-[85vh] overflow-y-auto">
            ...
            <button
              onClick={() => setActiveTutorial(null)}
              className="w-full py-4 bg-primary text-white rounded-2xl font-bold mt-4 active:scale-95 transition-transform"
            >
              Got it!
            </button>
          </div>
